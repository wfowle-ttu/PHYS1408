...
Reference [The first lecture for kinematic equations](./lecture_82626.qmd#kinematic-equation-1).
...
![Example problem 4 visualization](Lec2ExProblem4.png)
...
Removing the variables we know equate to zero our equation shifts:
$$
y_f = y_i + \frac{1}{2}(-g)t^2
$$

And the final position of y is the depth of our well.
$$
y_f = \frac{1}{2}(-g)t^2
$$
...
Let's shuffle our previous problem slightly to reveal additional algebra within the [4th Kinematic equation](./lecture_82626.qmd#kinematic-equation-4).
...
$$
y_f = y_i + v_{yi}*t + \frac{1}{2}a_yt^2-V_{yi}t
$$
...
a_y = -g \newline
y_f = 0 \newline
y_i = \space ? \space \text{(also known as h)}
$$
...
$$
0 = h + V_{yi}t - \frac{gt^2}{2}
$$

Which simplifies to:

$$
h = \frac{gt^2}{2}-V_{yi}t \newline
h = 9.8/2*3^2-5*3 \newline
h = 29.1 \space \text{meters deep}
$$
...
![Example problem 1 visualized](Lec2Ex1.drawio.png)
...
Firstly since the ball is rolling across the $x$ axis at 1m/s and  we know the x axis takes on no additional acceleration we can calculate the x position of the ball using one of the kinematic equations, [specifically equation 4:](./lecture_82626.qmd#kinematic-equation-4)
